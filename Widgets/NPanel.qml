...
Loader {
    ...
    readonly property bool barIsVertical: barPosition === "left"
                                          || barPosition === "right"
    ...
    readonly property bool effectivePanelAnchorTop: panelAnchorTop
                                                    || (useButtonPosition
                                                        && barPosition === "top")
    readonly property bool effectivePanelAnchorBottom: panelAnchorBottom
                                                       || (useButtonPosition
                                                           && barPosition === "bottom")
    readonly property bool effectivePanelAnchorLeft: panelAnchorLeft
                                                     || (useButtonPosition
                                                         && barPosition === "left")
    readonly property bool effectivePanelAnchorRight: panelAnchorRight
                                                      || (useButtonPosition
                                                          && barPosition === "right")
    ...
    sourceComponent: Component {
        ...
        PanelWindow {
            ...
            readonly property bool barIsVisible: (screen !== null)
                                                 && (Settings.data.bar.monitors.includes(
                                                         screen.name)
            ...
            visible: true
            color: Settings.data.general.dimDesktop ? Qt.alpha(
                                                          Color.mShadow,
                                                          dimmingOpacity) : Color.transparent
            ...
            NShapedRectangle {
                ...
                width: {
                    ...
                    if (root.preferredWidthRatio !== undefined) {
                        w = Math.round(
                                    Math.max(
                                        screen?.width * root.preferredWidthRatio,
                                        root.preferredWidth))
                    } else {
                ...
                height: {
                    ...
                    if (root.preferredHeightRatio !== undefined) {
                        h = Math.round(
                                    Math.max(
                                        screen?.height * root.preferredHeightRatio,
                    ...
                    return Math.min(
                                h,
                                screen?.height - Style.barHeight - Style.marginL * 2)
                }
                ...
                property int calculatedX: {
                    ...
                    if (barIsVertical) {
                        ...
                        if (root.useButtonPosition) {
                            ...
                            var targetX = buttonPosition.x + (buttonWidth / 2)
                                    - (panelBackground.width / 2)
                ...
                property int calculatedY: {
                    ...
                    if (barIsVertical) {
                        ...
                        if (useButtonPosition) {
                            // Position panel relative to button
                            var targetY = buttonPosition.y + (buttonHeight / 2)
                                    - (panelBackground.height / 2)
                            // Keep panel within screen bounds
                            var maxY = panelWindow.height - panelBackground.height - marginBottom
                            var minY = marginTop
                    ...
                    onItemChanged: {
                        if (item) {
                            ...
                            if (item.hasOwnProperty('contentPreferredWidth')) {
                                root.preferredWidth = Qt.binding(
                                            () => item.contentPreferredWidth)
                            }
                            if (item.hasOwnProperty('contentPreferredHeight')) {
                                root.preferredHeight = Qt.binding(
                                            () => item.contentPreferredHeight)
                            }
                            if (item.hasOwnProperty(
                                        'contentPreferredWidthRatio')) {
                                root.preferredWidthRatio = Qt.binding(
                                            () => item.contentPreferredWidthRatio)
                            }
                            if (item.hasOwnProperty(
                                        'contentPreferredHeightRatio')) {
                                root.preferredHeightRatio = Qt.binding(
                                            () => item.contentPreferredHeightRatio)
                            }
                        }
                    }
                }
                ...
                DragHandler {
                    ...
                    onTranslationChanged: {
                        ...
                        var insetLeft = baseGap + ((barIsVisible
                                                    && barPosition === "left") ? (Style.barHeight + floatExtraH) : 0)
                        var insetRight = baseGap + ((barIsVisible
                                                     && barPosition === "right") ? (Style.barHeight + floatExtraH) : 0)
                        var insetTop = baseGap + ((barIsVisible && barPosition
                                                   === "top") ? (Style.barHeight + floatExtraV) : 0)
                        var insetBottom = baseGap + ((barIsVisible
                                                      && barPosition === "bottom") ? (Style.barHeight + floatExtraV) : 0)
                        ...
                        panelBackground.manualX = Math.round(
                                    Math.max(minX, Math.min(nx, maxX)))
                        panelBackground.manualY = Math.round(
                                    Math.max(minY, Math.min(ny, maxY)))
                    }
                }
