...
NPanel {
    ...
    panelContent: Item {
        ...
        property real contentPreferredHeight: {
            ...
            const numWeeks = grid.daysModel ? Math.ceil(
                                                  grid.daysModel.length / 7) : 5

            // Base heights
            const bannerHeight = 90 * Style.uiScaleRatio
            const navigationHeight = 50 * Style.uiScaleRatio
            const dayNamesHeight = 35 * Style.uiScaleRatio
            ...
            const calendarGridHeight = numWeeks * rowHeight

            // Weather card height
            const weatherHeight = (Settings.data.location.weatherEnabled
                                   && Settings.data.location.showCalendarWeather ? 210 : 0)
                                * Style.uiScaleRatio

            return bannerHeight + navigationHeight + dayNamesHeight
                    + calendarGridHeight + weatherHeight + margins
        }
        ...
        ColumnLayout {
            ...
            readonly property bool weatherReady: Settings.data.location.weatherEnabled
                                                 && (LocationService.data.weather !== null)

            function checkIsCurrentMonth() {
                return (Time.date.getMonth() === grid.month)
                        && (Time.date.getFullYear() === grid.year)
            }
                ...
                NIconButton {
                    ...
                    onClicked: {
                        ...
                        const daysBehind = Math.max(
                                             0, Math.ceil(
                                                 (now - monthStart) / (24 * 60 * 60 * 1000)))
                        const daysAhead = Math.max(
                                            0, Math.ceil(
                                                (monthEnd - now) / (24 * 60 * 60 * 1000)))

                        CalendarService.loadEvents(daysAhead + 30,
                                                   daysBehind + 30)
                    }
                }
                        ...
                        Item {
                            ...
                            NText {
                                ...
                                text: {
                                    ...
                                    const dayName = I18n.locale.dayName(
                                                      dayIndex,
                                                      Locale.ShortFormat)
            ...
            RowLayout {
                ...
                function hasEventsOnDate(year, month, day) {
                    if (!CalendarService.available
                            || CalendarService.events.length === 0)
                        return false

                    const targetDate = new Date(year, month, day)
                    const targetStart = new Date(targetDate.getFullYear(),
                                                 targetDate.getMonth(),
                                                 targetDate.getDate(
                                                     )).getTime() / 1000
                    ...
                    return CalendarService.events.some(event => {
                                                           ...
                                                           return (event.start >= targetStart
                                                                   && event.start < targetEnd)
                                                           || (event.end > targetStart
                                                               && event.end <= targetEnd)
                ...
                function isAllDayEvent(event) {
                    ...
                    const isAtMidnight = startDate.getHours() === 0
                ...
                function isMultiDayEvent(event) {
                    ...
                    const startDateOnly = new Date(startDate.getFullYear(),
                                                   startDate.getMonth(),
                                                   startDate.getDate())
                    const endDateOnly = new Date(endDate.getFullYear(),
                                                 endDate.getMonth(),
                                                 endDate.getDate())
                    ...
                    property var weekNumbers: {
                        ...
                        for (var i = 0; i < numWeeks; i++) {
                            ...
                            if (dayIndex < grid.daysModel.length) {
                                ...
                                const date = new Date(weekDay.year,
                                                      weekDay.month,
                                                      weekDay.day)
                ...
                GridLayout {
                    ...
                    Layout.fillWidth: true
                    Layout.preferredHeight: {
                        const numWeeks = daysModel ? Math.ceil(
                                                         daysModel.length / 7) : 5
                        const rowHeight = Style.baseWidgetSize * 0.9 + Style.marginXXS
                        return numWeeks * rowHeight
                    }
                    columns: 7
                    ...
                    property var daysModel: {
                        ...
                        for (var day = 1; day <= daysInMonth; day++) {
                            ...
                            const isToday = date.getFullYear(
                                              ) === today.getFullYear()
                    ...
                    Repeater {
                        ...
                        Item {
                            ...
                            Rectangle {
                                ...
                                Row {
                                    visible: Settings.data.location.showCalendarEvents
                                             && parent.parent.parent.parent.hasEventsOnDate(
                                                 modelData.year,
                                                 modelData.month, modelData.day)
                                    spacing: 2
                                    ...
                                    Repeater {
                                        model: parent.parent.parent.parent.parent.getEventsForDate(
                                                   modelData.year,
                                                   modelData.month,
                                                   modelData.day)
                                        ...
                                        Rectangle {
                                            ...
                                            height: width
                                            radius: width / 2
                                            color: parent.parent.parent.parent.parent.getEventColor(
                                                       modelData,
                                                       modelData.today)
                                        }
                                    }
                                ...
                                MouseArea {
                                    ...
                                    onEntered: {
                                        const events = parent.parent.parent.parent.getEventsForDate(
                                                         modelData.year,
                                                         modelData.month,
                                                         modelData.day)
                                        if (events.length > 0) {
                                            const summaries = events.map(
                                                                e => e.summary).join(
                                                                '\n')
                                            TooltipService.show(Screen, parent,
                                                                summaries)
                                    ...
                                    onClicked: {
                                        const dateWithSlashes = `${(modelData.month + 1).toString(
                                                                  ).padStart(
                                                                  2,
                                                                  '0')}/${modelData.day.toString(
                                                                  ).padStart(
                                                                  2,
                                                                  '0')}/${modelData.year.toString(
                                                                  ).substring(
                                                                  2)}`
                                        Quickshell.execDetached(
                                                    ["gnome-calendar", "--date", dateWithSlashes])
            ...
            Loader {
                ...
                active: Settings.data.location.weatherEnabled
                        && Settings.data.location.showCalendarWeather
                Layout.fillWidth: true
